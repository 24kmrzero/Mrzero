(async function(){
  ...
  function filteredClients(){
    ...
    return clients.filter(c=>{const hay=String(c.full_name||'').toLowerCase();if(q&&!hay.includes(q))return false;if(vf==='verified'&&!c.email_verified)return false;if(vf==='unverified'&&c.email_verified)return false;const enrolled=(c.enrollments||[]).length>0;if(ef==='enrolled'&&!enrolled)return false;if(ef==='not-enrolled'&&enrolled)return false;return true;});
  }

  function renderClients(){
    const clients=filteredClients();if($('teamClientCount'))$('teamClientCount').textContent=String(clients.length);
    $('teamClientsBody').innerHTML=clients.length?clients.map(c=>`<tr><td><div class="team-client-main"><div class="team-client-avatar">${esc(initials(c.full_name||'ST'))}</div><div><b>${esc(c.full_name||'Student')}</b><small>Client details protected</small></div></div></td><td>${c.whatsapp_url?`<a class="app-btn small green team-whatsapp-btn" href="${esc(c.whatsapp_url)}" target="_blank" rel="noopener"><i class="fa-brands fa-whatsapp"></i> WhatsApp</a>`:'<span class="muted">Unavailable</span>'}</td><td>${fmt(c.signup_date)}</td><td><span class="status-pill ${c.email_verified?'ok':'warn'}">${c.email_verified?'Verified':'Unverified'}</span></td><td>${(c.enrollments||[]).length?(c.enrollments||[]).map(e=>`<div class="team-course-line"><b>${esc(e.course_title||'Course')}</b><span class="status-pill ${String(e.status).toLowerCase()==='active'?'ok':'neutral'}">${esc(e.status||'')}</span></div>`).join(''):'<span class="muted">Not enrolled</span>'}</td></tr>`).join(''):'<tr><td colspan="5"><div class="team-empty"><i class="fa-solid fa-users-slash"></i><b>No matching clients</b><span>Clients attributed to your assigned link will appear here.</span></div></td></tr>';
  }
