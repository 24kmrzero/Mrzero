(function(){
  'use strict';
  if (window.__24K_V971_COURSE_FIX__) return;
  window.__24K_V971_COURSE_FIX__ = true;
  ...
  async function deleteCourse(id,button){
    const course=state().courses?.find(c=>String(c.id)===String(id));
    if(!course)return toast('Course could not be found.','error');
    let confirmed=false;
    if(typeof A.confirmAction==='function'){
      const result=await A.confirmAction({title:'Delete Course',message:`Delete “${course.title}” and its connected sessions? This action cannot be undone.`,confirmText:'Delete Permanently',danger:true});
      confirmed=Boolean(result?.confirmed);
    }else confirmed=window.confirm(`Delete “${course.title}”? This action cannot be undone.`);
    if(!confirmed)return;
    const old=button?.innerHTML;
    if(button){button.disabled=true;button.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>';}
    try{
      const res=await A.supabase.from('courses').delete().eq('id',id);
    ...
    }catch(error){toast(typeof A.friendlyError==='function'?A.friendlyError(error,'Could not delete course.'):String(error?.message||error),'error');}
