(function(){
  'use strict';
  if(window.__24K_V990_STUDENT_FIXES__)return;
  window.__24K_V990_STUDENT_FIXES__=true;
  ...
  function injectStyle(){
    if(document.getElementById('v990-student-style'))return;
    const s=document.createElement('style');s.id='v990-student-style';s.textContent=`
  ...
  let tm=0;const schedule=()=>{clearTimeout(tm);tm=setTimeout(init,35);};
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',schedule,{once:true});else schedule();
  window.addEventListener('load',schedule,{once:true});
  window.addEventListener('hashchange',schedule);
  window.addEventListener('24k:student-base-updated',schedule);
  // Delegated fallback keeps dynamically rendered Manage Access buttons working without observing every DOM mutation.
  document.addEventListener('click',event=>{
    const btn=event.target instanceof Element?event.target.closest('button,a,[role="button"]'):null;
    if(!btn||lower(btn).replace(/\s+/g,' ')!=='manage access')return;
    setTimeout(()=>{if(!accessUiVisible())fallbackManage(btn);},180);
  },true);
  [350,900,1900,3800].forEach(ms=>setTimeout(schedule,ms));
})();
