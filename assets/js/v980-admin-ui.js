(function(){
  'use strict';
  if (window.__24K_V980_ADMIN_UI__) return;
  window.__24K_V980_ADMIN_UI__ = true;
  ...
  function injectStyles(){
    if (document.getElementById('v980-admin-style')) return;
    const style = document.createElement('style');
    style.id = 'v980-admin-style';
    style.textContent = `
  ...
  function reorderNav(sidebar){
    if (!sidebar) return;
    const navs = qsa('nav',sidebar).filter(n => scoreSidebar(n)>=5);
    const nav = navs[0];
    if (!nav || nav.dataset.v980Ordered) return;
    const desired = [
      ['Overview',['Dashboard']],
      ['Content',['Signals','Charts','Articles','Announcements']],
      ['Courses',['Courses','Zoom Sessions']],
      ['Operations',['Premium Access','Link Manager','Admin Notifications','Activity Logs']]
    ];
    const controls = qsa('a,button,[role="button"]',nav);
    const picked = new Map();
    desired.flatMap(g=>g[1]).forEach(label=>{
      const key = label.toLowerCase();
      const match = controls.find(el=>{
        const t = lower(el).replace(/\b\d+\b/g,'').replace(/\s+/g,' ').trim();
        return t === key || t.startsWith(key+' ') || (key==='courses' && t==='courses');
      });
      if (match) picked.set(label,directNavItem(match,nav));
    });
    if (picked.size < 7) return;
    qsa(':scope > *',nav).forEach(ch=>{
      const t = text(ch).toLowerCase();
      if (['overview','content','courses','operations'].includes(t) && !Array.from(picked.values()).includes(ch)) ch.remove();
    });
    const moved = new Set();
    desired.forEach(([group,items])=>{
      const lab = document.createElement('div');
      lab.className='v980-nav-label';
      lab.textContent=group;
      nav.appendChild(lab);
      items.forEach(name=>{
        const item=picked.get(name);
        if(item && !moved.has(item)){ nav.appendChild(item); moved.add(item); }
      });
    });
    nav.dataset.v980Ordered='1';
  }
  ...
  function wireModalActions(){
    if(window.__24K_V980_MODAL_ACTIONS__)return;
    window.__24K_V980_MODAL_ACTIONS__=true;
  ...
  function closeModalOnSuccess(){
    if(window.__24K_V980_SUCCESS_WATCH__)return;
    window.__24K_V980_SUCCESS_WATCH__=true;
  ...
  let timer=0;
  function schedule(){clearTimeout(timer);timer=setTimeout(init,45);}
