(async function () {
  ...
  async function sendBulkMessage(event) {
    ...
    try {
      ...
      if (response.error) throw response.error;
      if (Boolean(form.elements.send_email.checked)) await A.supabase.functions.invoke('process-email-queue', { body: { limit: 50, retry_failed: true } }).catch(() => null);
  ...
  async function queueAdminVerification(userId) {
    ...
    const {error}=await A.supabase.auth.resend({type:'signup',email,options:{emailRedirectTo:`${window.location.origin}/sign-in/?verified=1`}});
    if(error)throw error;
    return {success:true,message:'Supabase verification email sent.'};
  }
