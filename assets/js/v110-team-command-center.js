...
let payload=null,rangeData=null,chatFilter='all',chatSearch='',activeThread=null,chatList=[],installPrompt=null;
...
async function load(){if(!token())return loginView();const alreadyOpen=document.body.classList.contains('team-authenticated');if(alreadyOpen)setHomeLoading(true);try{const month=$('#teamMonth')?.value||monthNow();payload=await commandCenter(month);appView();renderAll();await loadRange('today')}catch(err){console.error(err);if(/session|expired|invalid/i.test(err.message||'')){localStorage.removeItem(TOKEN_KEY);loginView('Your Team session expired. Please sign in again.')}else loginView('Could not load Team Panel: '+(err.message||'Unknown error'))}finally{setHomeLoading(false)}}
function renderAll(){const a=payload.account||{},p=payload.performance||{};const name=a.display_name||a.username||'Team Member';$('#teamMemberName').textContent=name;$('#heroName').textContent=name;$('#teamAvatar').textContent=initials(name);if($('#teamHomeAvatar'))$('#teamHomeAvatar').textContent=initials(name);if($('#teamGreeting')){const hr=new Date().getHours();$('#teamGreeting').textContent=hr<12?'Good morning':hr<17?'Good afternoon':'Good evening'}$('#teamUsername').textContent='@'+(a.username||'team');$('#teamMonthLabel').textContent=monthLabel($('#teamMonth').value);renderOverview();renderClients();renderDaily();renderEarnings(p);renderHistory(payload.history||[]);renderLinks(payload.links||[]);refreshHomeChatState().catch(()=>{})}
...
async function loadChat(){if(!token())return;try{const [summary,list]=await Promise.all([rpc('team_live_desk_summary_v12_18',{p_token:token()}),rpc('team_live_desk_list_v12_18',{p_token:token(),p_filter:chatFilter,p_search:chatSearch||null})]);chatList=list||[];$('#chatKpis').innerHTML=[['AI Active',summary.ai_active||0],['Human Requests',summary.human_requests||0],['Waiting',summary.waiting||0],['My Chats',summary.my_chats||0]].map(x=>`<div class="chat-kpi"><span>${x[0]}</span><b>${x[1]}</b></div>`).join('');const attention=(summary.human_requests||0)+(summary.waiting||0);$('#teamChatBadge').hidden=!attention;$('#teamChatBadge').textContent=attention;setMobileBadge('#teamMobileChatBadge',attention);renderChatList();if(activeThread&&chatList.some(x=>x.id===activeThread))await openChat(activeThread,false);else if(chatList[0])await openChat(chatList[0].id,false);else clearChat()}catch(e){if(/function|does not exist|schema cache/i.test(e.message||'')){$('#chatKpis').innerHTML='<div class="panel-card chat-empty" style="grid-column:1/-1">Live Desk update requires V12.18 SQL.</div>';return}throw e}}
