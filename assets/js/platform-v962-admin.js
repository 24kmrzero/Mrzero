(async function(){
  ...
  function renderIb(){const b=document.getElementById('premiumIbBody');if(!b)return;b.innerHTML=state.ib.length?state.ib.map(r=>`<tr><td><b>${esc(profileName(r.student_id))}</b></td><td>${esc(r.broker)}</td><td>${esc(r.trading_account_id)}<small>${esc(r.account_type==='existing'?'Partner / IB Shift':'New Account')}</small></td><td>${r.deposit_amount!=null?Number(r.deposit_amount).toLocaleString('en-US',{maximumFractionDigits:2}):'—'}</td><td>${A.formatDateTime(r.created_at)}</td><td><span class="status-pill ${A.statusClass(r.status)}">${A.statusLabel(r.status)}</span></td><td><div class="table-actions">${r.deposit_proof_path?`<button class="app-btn small outline" data-ib-proof="${r.id}" data-ib-proof-type="deposit">Deposit</button>`:''}${(r.broker_confirmation_proof_path||r.proof_path)?`<button class="app-btn small outline" data-ib-proof="${r.id}" data-ib-proof-type="confirmation">Confirmation</button>`:''}${!r.deposit_proof_path&&!r.proof_path&&!r.broker_confirmation_proof_path?'—':''}</div></td><td><div class="table-actions">${r.status==='pending'?`<button class="app-btn small gold" data-ib-review="${r.id}" data-ib-status="approved">Approve</button><button class="app-btn small danger" data-ib-review="${r.id}" data-ib-status="declined">Decline</button>`:'—'}</div></td></tr>`).join(''):'<tr><td colspan="8"><div class="empty-state compact">No IB verification requests.</div></td></tr>';}
  ...
  function bind(){
    ...
    document.body.addEventListener('click',async e=>{
      ...
      const proof=e.target.closest('[data-ib-proof]');if(proof)await openProof(proof.dataset.ibProof,proof.dataset.ibProofType||'confirmation');
    });
  }
  ...
  async function openProof(id,type='confirmation'){const row=state.ib.find(r=>r.id===id);if(!row)return;const path=type==='deposit'?row.deposit_proof_path:(row.broker_confirmation_proof_path||row.proof_path);if(!path)return A.toast('This proof is not available.','warning');const {data,error}=await A.supabase.storage.from('ib-proofs').createSignedUrl(path,120);if(error)return A.toast(A.friendlyError(error),'error');window.open(data.signedUrl,'_blank','noopener');}
  async function flushEmailQueue(){try{const r=await A.supabase.functions.invoke('process-email-queue',{body:{limit:50,retry_failed:true}});if(r.error)throw r.error;return r.data||null;}catch(e){console.warn('Email queue delivery failed:',e?.message||e);return null;}}
