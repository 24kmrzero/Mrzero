(async function () {
  ...
  function signalIsFinal(s){const st=String(s?.status||'');return Boolean(s?.closed_at)||FINAL_SIGNAL_STATES.has(st)||(st==='tp3_hit'&&(s?.take_profit_4===null||s?.take_profit_4===undefined||s?.take_profit_4===''));}
  function signalProgress(s){const max=s.take_profit_4?4:3;return Math.min(100,Math.round((Math.min(max,Number(s.tp_hit||0))/max)*100));}
  ...
  function adminSignalDateParts(value){const d=value?new Date(value):null;if(!d||Number.isNaN(d.getTime()))return{date:'—',time:'—'};return{date:new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Karachi',day:'2-digit',month:'short',year:'numeric'}).format(d),time:new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Karachi',hour:'2-digit',minute:'2-digit',hour12:true}).format(d)};}
  function adminSignalInstrumentMeta(symbol){const x=String(symbol||'').replace('/','').toUpperCase();const map={XAUUSD:{icon:'<i class="fa-solid fa-coins"></i>',name:'Gold / USD',tone:'gold'},XAGUSD:{icon:'<i class="fa-solid fa-circle-half-stroke"></i>',name:'Silver / USD',tone:'silver'},EURUSD:{icon:'€',name:'Euro / USD',tone:'blue'},GBPUSD:{icon:'£',name:'GBP / USD',tone:'navy'},AUDUSD:{icon:'A$',name:'AUD / USD',tone:'blue'},USDJPY:{icon:'¥',name:'USD / JPY',tone:'red'},BTCUSD:{icon:'₿',name:'Bitcoin / USD',tone:'orange'}};return map[x]||{icon:'<i class="fa-solid fa-chart-line"></i>',name:displaySymbol(x),tone:'gold'};}
  function adminLatestSignalPips(signal){if(signal.result_pips!==null&&signal.result_pips!==undefined)return Number(signal.result_pips);const update=state.signalUpdates.find(u=>u.signal_id===signal.id&&u.result_pips!==null&&u.result_pips!==undefined);return update?Number(update.result_pips):null;}
  function renderSignals(){
    ...
    if(view==='history'){
      table.classList.add('admin-signal-history-report');table.classList.remove('admin-signal-user-style');
    ...
    table.classList.add('admin-signal-user-style');
    table.querySelector('thead').innerHTML='<tr><th>Time</th><th>Date</th><th>Instrument</th><th>Type</th><th>Entry</th><th>SL</th><th>TP1</th><th>TP2</th><th>TP3</th><th>Current Pips</th><th>Status</th><th>Actions</th></tr>';
    document.getElementById('signalsBody').innerHTML=rows.length?rows.map(s=>{const when=adminSignalDateParts(s.published_at||s.created_at);const meta=adminSignalInstrumentMeta(s.symbol);const pips=adminLatestSignalPips(s);const pipClass=pips==null?'':pips>0?'result-positive':pips<0?'result-negative':'result-neutral';const pipText=pips==null?'—':`${signed(pips)} Pips`;const order=A.statusLabel(s.order_type||'market');return `<tr class="admin-signal-user-row"><td class="admin-signal-time"><b>${A.escapeHtml(when.time)}</b></td><td class="admin-signal-date">${A.escapeHtml(when.date)}</td><td><div class="admin-instrument-cell"><span class="admin-instrument-badge ${meta.tone}">${meta.icon}</span><div><b>${A.escapeHtml(displaySymbol(s.symbol))}</b><small>${A.escapeHtml(meta.name)}</small></div></div></td><td><span class="table-direction ${String(s.direction||'').toLowerCase()}">${A.escapeHtml(s.direction||'—')}</span><small class="admin-order-type">${A.escapeHtml(order)}</small></td><td><b>${entryText(s)}</b></td><td>${num(s.stop_loss)}</td><td>${num(s.take_profit_1)}</td><td>${num(s.take_profit_2)}</td><td>${num(s.take_profit_3)}</td><td><b class="admin-current-pips ${pipClass}">${A.escapeHtml(pipText)}</b></td><td><span class="status-pill ${A.statusClass(s.status)}">${A.statusLabel(s.status)}</span>${s.be_moved?'<small class="admin-be-note"><i class="fa-solid fa-shield-halved"></i> SL at BE</small>':''}</td><td><div class="table-actions admin-signal-actions"><button class="app-btn small outline" data-signal-history="${s.id}">History</button><button class="app-btn small gold" data-signal-update="${s.id}">Update</button><button class="app-btn small outline" data-edit="signal" data-id="${s.id}">Edit</button></div></td></tr>`}).join(''):`<tr><td colspan="12">${empty('No active signals found.','fa-bolt')}</td></tr>`;
  }
  ...
  function renderPayments(){
    ...
    document.getElementById('adminPaymentsBody').innerHTML=rows.length?rows.map(p=>{
      const providerInfo='';
      const receiptAction=p.receipt_path?`<button class="app-btn small outline" data-view-receipt="${p.id}">Receipt</button>`:'';
  ...
  function renderMethods(){
    const toggle=(m)=>`<button type="button" class="app-btn small method-toggle-btn ${m.is_active?'is-on':'is-off'}" data-toggle-method="${m.id}" data-next-active="${m.is_active?'false':'true'}"><i class="fa-solid fa-circle"></i> ${m.is_active?'ON':'OFF'}</button>`;
    const rows=state.methods.map(m=>{
      const isBank=/^local bank transfer$/i.test(String(m.name||'').trim());
      return `<tr><td><b>${A.escapeHtml(m.name)}</b>${isBank?'<small class="muted" style="display:block">Manual Admin Review</small>':''}</td><td>${A.escapeHtml(m.account_title||'')}</td><td>${A.escapeHtml(m.account_number||'')}</td><td>${A.escapeHtml(m.instructions||'')}</td><td>${toggle(m)}</td><td><span class="status-pill ${m.is_active?'ok':'warn'}">${m.is_active?'Active':'Inactive'}</span></td><td><div class="table-actions"><button class="app-btn small outline" data-edit="method" data-id="${m.id}">Edit</button>${isBank?'':`<button class="app-btn small danger" data-delete="method" data-id="${m.id}">Delete</button>`}</div></td></tr>`;
    }).join('');
  ...
  function finalTpAction(s,action){if(action==='tp1_hit')return !s.take_profit_2&&!s.take_profit_3&&!s.take_profit_4;if(action==='tp2_hit')return !s.take_profit_3&&!s.take_profit_4;if(action==='tp3_hit')return !s.take_profit_4;return action==='tp4_hit'}function availableActions(s){const list=[];const hit=Number(s.tp_hit||0);if(!s.be_moved)list.push(['move_to_be','Move SL to Breakeven']);if(s.take_profit_1&&hit<1)list.push(['tp1_hit',finalTpAction(s,'tp1_hit')?'TP1 Hit & Close':'TP1 Hit']);if(s.take_profit_2&&hit<2)list.push(['tp2_hit',finalTpAction(s,'tp2_hit')?'TP2 Hit & Close':'TP2 Hit']);if(s.take_profit_3&&hit<3)list.push(['tp3_hit',finalTpAction(s,'tp3_hit')?'TP3 Hit & Close':'TP3 Hit']);if(s.take_profit_4&&hit<4)list.push(['tp4_hit','TP4 Hit & Close']);list.push(['sl_hit','SL Hit'],['breakeven_hit','Breakeven Hit'],['manually_closed','Close Trade Manually'],['cancelled','Cancel Signal']);return list;}
  ...
  function automaticPipsForAction(signal,action,closePrice=null){
    ...
    const prices={tp1_hit:Number(signal.take_profit_1),tp2_hit:Number(signal.take_profit_2),tp3_hit:Number(signal.take_profit_3),tp4_hit:Number(signal.take_profit_4),sl_hit:Number(signal.stop_loss),breakeven_hit:entry,manually_closed:(closePrice==null||String(closePrice).trim()===''?NaN:Number(closePrice))};
  ...
  function renderStatusPreview(){
    ...
    const label={tp1_hit:finalTpAction(s,'tp1_hit')?'TP1 final result — signal will close':'TP1 current result',tp2_hit:finalTpAction(s,'tp2_hit')?'TP2 final result — signal will close':'TP2 current result',tp3_hit:finalTpAction(s,'tp3_hit')?'TP3 final result — signal will close':'TP3 current result',tp4_hit:'TP4 final result — signal will close',sl_hit:'Final Stop Loss result',breakeven_hit:'Final Breakeven result',manually_closed:'Manual close result',move_to_be:'Trade remains active; no Pip result required',cancelled:'Signal will close as Cancelled; no Pip result required'}[action]||'Select an action';
  ...
  async function saveSignalStatus(e){
    ...
    try{
      const {data,error}=await A.supabase.rpc('admin_update_signal_status_v969',{p_signal_id:v.signal_id,p_action:v.action,p_close_price:n(v.close_price),p_note:v.note||null,p_notify_users:checked(f,'notify_users')});
      ...
      renderAll();
      A.toast(data?.final?'Signal closed. Active/History counts updated without changing your tab.':'Signal status and Pips updated automatically.','success');
  ...
  async function togglePaymentMethod(id,nextActive,button){
    if(!id)return;
    A.setLoading(button,true,nextActive?'Turning on...':'Turning off...');
    try{
      const {error}=await A.supabase.from('payment_methods').update({is_active:Boolean(nextActive),updated_at:new Date().toISOString()}).eq('id',id);
      if(error)throw error;
      await loadAll();renderAll();
      A.toast(`Payment method turned ${nextActive?'ON':'OFF'}.`,'success');
    }catch(error){A.toast(A.friendlyError(error,'Could not update payment method.'),'error');}
    finally{A.setLoading(button,false);}
  }
