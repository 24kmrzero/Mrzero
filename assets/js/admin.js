(async function () {
  ...
  async function saveSignalStatus(e){
    ...
    try{
      ...
      renderAll();
      A.toast(['tp4_hit','sl_hit','breakeven_hit','manually_closed','cancelled'].includes(v.action)||(v.action==='tp3_hit'&&!state.signals.find(x=>x.id===v.signal_id)?.take_profit_4)?'Signal closed. Active/History counts updated without changing your tab.':'Signal status and Pips updated automatically.','success');
