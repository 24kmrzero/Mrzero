(async function () {
  ...
  function renderMethods(){
    const local=state.methods.find(m=>/^(local bank transfer|infinity(?: money solutions)?)$/i.test(String(m.name||'').trim()));
    const toggle=(m)=>m?`<button type="button" class="app-btn small method-toggle-btn ${m.is_active?'is-on':'is-off'}" data-toggle-method="${m.id}" data-next-active="${m.is_active?'false':'true'}"><i class="fa-solid fa-circle"></i> ${m.is_active?'ON':'OFF'}</button>`:`<span class="status-pill warn">Setup SQL</span>`;
    const builtIn=`<tr><td><b>Local Bank Transfer</b><small class="muted" style="display:block">Built-in</small></td><td>Automatic</td><td>—</td><td>Secure hosted local bank transfer for PKR course payments.</td><td>${toggle(local)}</td><td><span class="status-pill ${local?.is_active?'ok':'warn'}">${local?.is_active?'Active':'Inactive'}</span></td><td><span class="status-pill neutral"><i class="fa-solid fa-lock"></i> Built-in</span></td></tr>`;
    const editable=state.methods.filter(m=>!/^(local bank transfer|infinity(?: money solutions)?)$/i.test(String(m.name||'').trim()));
    const rows=editable.map(m=>`<tr><td><b>${A.escapeHtml(m.name)}</b></td><td>${A.escapeHtml(m.account_title||'')}</td><td>${A.escapeHtml(m.account_number||'')}</td><td>${A.escapeHtml(m.instructions||'')}</td><td>${toggle(m)}</td><td><span class="status-pill ${m.is_active?'ok':'warn'}">${m.is_active?'Active':'Inactive'}</span></td><td><div class="table-actions"><button class="app-btn small outline" data-edit="method" data-id="${m.id}">Edit</button><button class="app-btn small danger" data-delete="method" data-id="${m.id}">Delete</button></div></td></tr>`).join('');
