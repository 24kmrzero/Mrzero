(async function () {
  ...
  function courseCard(course) {
    ...
    return `<article class="big public-course-card reveal ${isFree ? 'free' : ''}">
      ...
      <div class="big-head"><div><h3>${escapeHtml(course.title)}</h3><p class="big-sub">— Instructor: ${escapeHtml(course.instructor_name || 'Mr. Zameer')}</p><p class="big-desc">${escapeHtml(course.short_description || course.description || '')}</p></div></div>
