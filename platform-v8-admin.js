(async function () {
  ...
  function renderAll() {
    populateCourseOptions(); renderStructure(); renderEnrollments(); renderEnquiries(); renderLinks(); renderUsers(); renderDelivery();
    renderAdminNotifications(); renderAudit(); renderCalendar(); renderSettings(); renderOperationsDashboard();
    decorateBaseTables(); renderGlobalSearch();
  }

  function renderBaseDependentViews() {
    populateCourseOptions(); renderEnrollments(); renderUsers(); renderCalendar(); renderOperationsDashboard(); decorateBaseTables();
  }
  ...
  function renderLinks() { const body = document.getElementById('linkManagerBody'); if (!body) return; body.innerHTML = state.links.length ? state.links.map(row => { const url = trackedUrl(row); return `<tr><td><b>${esc(row.name)}</b><small class="truncate-url">${esc(url)}</small></td><td>${esc(row.team_member || '—')}<small>${esc(row.ref_code)}</small></td><td>${esc(row.source)}<small>${esc(row.campaign || '—')}</small></td><td>${row.total_clicks || 0}</td><td>${row.unique_visitors || 0}</td><td>${row.signups || 0}</td><td>${row.enrollments || 0}</td><td>${Number(row.conversion_rate || 0).toFixed(1)}%</td><td>${A.formatDateTime(row.last_activity)}</td><td><span class="status-pill ${row.is_active ? 'ok' : 'bad'}">${row.is_active ? 'Active' : 'Disabled'}</span></td><td><div class="table-actions"><button class="app-btn small gold" data-copy-link="${attr(url)}">Copy</button><button class="app-btn small outline" data-edit-link="${row.id}">Edit</button><button class="app-btn small outline" data-toggle-link="${row.id}">${row.is_active ? 'Disable' : 'Enable'}</button></div></td></tr>`; }).join('') : `<tr><td colspan="11">${empty('No tracked links created.', 'fa-link')}</td></tr>`; renderLinkSummary(); }
